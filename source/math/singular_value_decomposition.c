...
static void Householders_Reduction_to_Bidiagonal_Form(double* A, unsigned int nrows,
    unsigned int ncols, double* U, double* V, double* diagonal, double* superdiagonal );
static int  Givens_Reduction_to_Diagonal_Form( unsigned int nrows, unsigned int ncols,
           double* U, double* V, double* diagonal, double* superdiagonal );
static void Sort_by_Decreasing_Singular_Values(unsigned int nrows, unsigned int ncols,
...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
int smatrix_Singular_Value_Decomposition(
    double* A, 
    unsigned int nrows, 
    unsigned int ncols, 
    double* U, 
...
//     The default tolerance is D[0] * EPSI * ncols, assuming that the //
...
void smatrix_Singular_Value_Decomposition_Inverse(
    double* U, 
    double* D, 
    double* V,  
    double tolerance, 
...
ERROR_CODE matrix_pseudoinverse(MATRIX a, MATRIX *output) {
    ...
    double A[a.rows][a.cols];
    double U[a.rows][a.cols];
    double D[a.cols];  
    double V[a.cols][a.cols];
    double* dummy_array;
    double Astar[a.cols][a.rows]; 

    dummy_array = (double*) malloc(a.cols * sizeof(double));
    if (NULL == dummy_array) {
    ...
    smatrix_Singular_Value_Decomposition_Inverse((double*)U, D,(double*)V,0.0,a.rows,a.cols, (double*)Astar);
    for (int i = 0; i < output->rows; i++) {
        for (int j = 0; j < output->cols; j++) {
            output->data[i][j] = Astar[i][j];
        }
    }
...
int main(int argc, char **argv) {
   ...
   A[1][0] = 0.0;    A[1][1] = 1.0;    A[1][2] =-3.0;

   memset(U,0,rows*cols*sizeof(double));

   // Compute SVD
   if (0 != smatrix_Singular_Value_Decomposition((double*)A,rows,cols,(double*)U,D,(double*)V,dummy_array)) {
      printf("Failed to perform Singular Value Decomposition for pseudoinverse\n");
      return 1;
   }
   free(dummy_array);
   // Compute inverse
   smatrix_Singular_Value_Decomposition_Inverse((double*)U, D,(double*)V,0.0,rows,cols, (double*)Astar);
