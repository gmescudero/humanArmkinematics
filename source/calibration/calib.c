...
/**
 * @brief Compute the error value given the values of angular velocity and the rotation vectors
 * 
 * @note Code generated by sympy
 * 
 * @param omegaR (input) Angular velocity measures
 * @param j1 (input) First rotation vector
 * @param j2 (input) Second rotation vector
 * @return double (error) The error value 
 */
static double scal_error_compute(double omegaR[3], double j1[3], double j2[3]) {
    double omega_x = omegaR[0], omega_y = omegaR[1], omega_z = omegaR[2]; 
    double j_x1 = j1[0], j_y1 = j1[1], j_z1 = j1[2]; 
    double j_x2 = j2[0], j_y2 = j2[1], j_z2 = j2[2]; 
    return (omega_x*(j_y1*j_z2 - j_y2*j_z1) + omega_y*(-j_x1*j_z2 + j_x2*j_z1) + omega_z*(j_x1*j_y2 - j_x2*j_y1));
}

/**
 * @brief Compute the error derivatives given the angular velocities and the spherical coordinates
 * 
 * @note Code generated by sympy
 * 
 * @param omegaR (input) Angular velocity measures
 * @param theta_1 (input) Theta angle of the spherical coordinates of the first rotation vector
 * @param rho_1 (input) Rho angle of the spherical coordinates of the first rotation vector
 * @param theta_2 (input) Theta angle of the spherical coordinates of the second rotation vector
 * @param rho_2 (input) Rho angle of the spherical coordinates of the second rotation vector
 * @param sph_alt1 (input) Spherical convention for first rotation vector
 * @param sph_alt2 (input) Spherical convention for second rotation vector
 * @param dErr (output) Vector of the four derivatives: de/dt1, de/dr1, de/dt2, de/dr2
 * @return ERROR_CODE 
 */
static ERROR_CODE scal_error_derivatives_compute(double omegaR[3], double theta_1, double rho_1, double theta_2, double rho_2, int sph_alt1, int sph_alt2, double *dErr) {
    double omega_x = omegaR[0], omega_y = omegaR[1], omega_z = omegaR[2]; 

    if      (0 == sph_alt1 && 0 == sph_alt2) {
        dErr[0] = omega_x*(sin(rho_1)*cos(theta_1)*cos(theta_2) + sin(rho_2)*sin(theta_1)*sin(theta_2)) + omega_y*(-sin(theta_1)*sin(theta_2)*cos(rho_2) - cos(rho_1)*cos(theta_1)*cos(theta_2)) + omega_z*(-sin(rho_1)*sin(theta_2)*cos(rho_2)*cos(theta_1) + sin(rho_2)*sin(theta_2)*cos(rho_1)*cos(theta_1));
        dErr[1] = omega_x*sin(theta_1)*cos(rho_1)*cos(theta_2) + omega_y*sin(rho_1)*sin(theta_1)*cos(theta_2) + omega_z*(-sin(rho_1)*sin(rho_2)*sin(theta_1)*sin(theta_2) - sin(theta_1)*sin(theta_2)*cos(rho_1)*cos(rho_2));
        dErr[2] = omega_x*(-sin(rho_1)*sin(theta_1)*sin(theta_2) - sin(rho_2)*cos(theta_1)*cos(theta_2)) + omega_y*(sin(theta_1)*sin(theta_2)*cos(rho_1) + cos(rho_2)*cos(theta_1)*cos(theta_2)) + omega_z*(-sin(rho_1)*sin(theta_1)*cos(rho_2)*cos(theta_2) + sin(rho_2)*sin(theta_1)*cos(rho_1)*cos(theta_2));
        dErr[3] = -omega_x*sin(theta_2)*cos(rho_2)*cos(theta_1) - omega_y*sin(rho_2)*sin(theta_2)*cos(theta_1) + omega_z*(sin(rho_1)*sin(rho_2)*sin(theta_1)*sin(theta_2) + sin(theta_1)*sin(theta_2)*cos(rho_1)*cos(rho_2));
    }
    else if (1 == sph_alt1 && 1 == sph_alt2) {
        dErr[0] = omega_x*(sin(rho_1)*sin(theta_2)*cos(rho_2)*cos(theta_1) - sin(rho_2)*sin(theta_2)*cos(rho_1)*cos(theta_1)) + omega_y*(sin(theta_1)*sin(theta_2)*cos(rho_2) + cos(rho_1)*cos(theta_1)*cos(theta_2)) + omega_z*(-sin(rho_1)*cos(theta_1)*cos(theta_2) - sin(rho_2)*sin(theta_1)*sin(theta_2));
        dErr[1] = omega_x*(sin(rho_1)*sin(rho_2)*sin(theta_1)*sin(theta_2) + sin(theta_1)*sin(theta_2)*cos(rho_1)*cos(rho_2)) - omega_y*sin(rho_1)*sin(theta_1)*cos(theta_2) - omega_z*sin(theta_1)*cos(rho_1)*cos(theta_2);
        dErr[2] = omega_x*(sin(rho_1)*sin(theta_1)*cos(rho_2)*cos(theta_2) - sin(rho_2)*sin(theta_1)*cos(rho_1)*cos(theta_2)) + omega_y*(-sin(theta_1)*sin(theta_2)*cos(rho_1) - cos(rho_2)*cos(theta_1)*cos(theta_2)) + omega_z*(sin(rho_1)*sin(theta_1)*sin(theta_2) + sin(rho_2)*cos(theta_1)*cos(theta_2));
        dErr[3] = omega_x*(-sin(rho_1)*sin(rho_2)*sin(theta_1)*sin(theta_2) - sin(theta_1)*sin(theta_2)*cos(rho_1)*cos(rho_2)) + omega_y*sin(rho_2)*sin(theta_2)*cos(theta_1) + omega_z*sin(theta_2)*cos(rho_2)*cos(theta_1);
    }
    else if (1 == sph_alt1 && 0 == sph_alt2) {
        dErr[0] = omega_x*(sin(rho_1)*cos(theta_1)*cos(theta_2) - sin(rho_2)*sin(theta_2)*cos(rho_1)*cos(theta_1)) + omega_y*(sin(theta_1)*cos(theta_2) + sin(theta_2)*cos(rho_1)*cos(rho_2)*cos(theta_1)) + omega_z*(-sin(rho_1)*sin(theta_2)*cos(rho_2)*cos(theta_1) - sin(rho_2)*sin(theta_1)*sin(theta_2));
        dErr[1] = omega_x*(sin(rho_1)*sin(rho_2)*sin(theta_1)*sin(theta_2) + sin(theta_1)*cos(rho_1)*cos(theta_2)) - omega_y*sin(rho_1)*sin(theta_1)*sin(theta_2)*cos(rho_2) - omega_z*sin(theta_1)*sin(theta_2)*cos(rho_1)*cos(rho_2);
        dErr[2] = omega_x*(-sin(rho_1)*sin(theta_1)*sin(theta_2) - sin(rho_2)*sin(theta_1)*cos(rho_1)*cos(theta_2)) + omega_y*(sin(theta_1)*cos(rho_1)*cos(rho_2)*cos(theta_2) + sin(theta_2)*cos(theta_1)) + omega_z*(-sin(rho_1)*sin(theta_1)*cos(rho_2)*cos(theta_2) + sin(rho_2)*cos(theta_1)*cos(theta_2));
        dErr[3] = -omega_x*sin(theta_1)*sin(theta_2)*cos(rho_1)*cos(rho_2) - omega_y*sin(rho_2)*sin(theta_1)*sin(theta_2)*cos(rho_1) + omega_z*(sin(rho_1)*sin(rho_2)*sin(theta_1)*sin(theta_2) + sin(theta_2)*cos(rho_2)*cos(theta_1));
    }
    else if (0 == sph_alt1 && 1 == sph_alt2) {
        dErr[0] = omega_x*(sin(rho_1)*sin(theta_2)*cos(rho_2)*cos(theta_1) + sin(rho_2)*sin(theta_1)*sin(theta_2)) + omega_y*(-sin(theta_1)*cos(theta_2) - sin(theta_2)*cos(rho_1)*cos(rho_2)*cos(theta_1)) + omega_z*(-sin(rho_1)*cos(theta_1)*cos(theta_2) + sin(rho_2)*sin(theta_2)*cos(rho_1)*cos(theta_1));
        dErr[1] = omega_x*sin(theta_1)*sin(theta_2)*cos(rho_1)*cos(rho_2) + omega_y*sin(rho_1)*sin(theta_1)*sin(theta_2)*cos(rho_2) + omega_z*(-sin(rho_1)*sin(rho_2)*sin(theta_1)*sin(theta_2) - sin(theta_1)*cos(rho_1)*cos(theta_2));
        dErr[2] = omega_x*(sin(rho_1)*sin(theta_1)*cos(rho_2)*cos(theta_2) - sin(rho_2)*cos(theta_1)*cos(theta_2)) + omega_y*(-sin(theta_1)*cos(rho_1)*cos(rho_2)*cos(theta_2) - sin(theta_2)*cos(theta_1)) + omega_z*(sin(rho_1)*sin(theta_1)*sin(theta_2) + sin(rho_2)*sin(theta_1)*cos(rho_1)*cos(theta_2));
        dErr[3] = omega_x*(-sin(rho_1)*sin(rho_2)*sin(theta_1)*sin(theta_2) - sin(theta_2)*cos(rho_2)*cos(theta_1)) + omega_y*sin(rho_2)*sin(theta_1)*sin(theta_2)*cos(rho_1) + omega_z*sin(theta_1)*sin(theta_2)*cos(rho_1)*cos(rho_2);
    }
    else {
        err_str("Invalid convination of spherical choices: %d,%d",sph_alt1,sph_alt2);
    ...
    double rotationV2[3])
{
    ...
    while (RET_OK == status && CALIB_TWO_ROT_AXES_MAX_ERROR < error && 0 < iterations) {
        ...
        for (int i = 0; RET_OK == status && i < observations_num; i++) {
            ...
            // Compute error
            if (RET_OK == status) {
                errorV.data[i][0] = scal_error_compute(wi,tempV1,tempV2_from1);
                squared_error += errorV.data[i][0]*errorV.data[i][0];
            }
            // Build Jacobian matrix row calculating partials of the error
            if (RET_OK == status) {
                status = scal_error_derivatives_compute(
                    wi, phi.data[0][0],phi.data[1][0],phi.data[2][0],phi.data[3][0], sph_alt1,sph_alt2, 
                    Jacobian.data[i]);
            }
        }
        ...
        if (RET_OK == status) {
            error = sqrt(squared_error)/observations_num;
        }
        // Compute rotation vector 2 from 1
        if (RET_OK == status) {
